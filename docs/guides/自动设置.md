...
`cat tmp.gen.keypair` 注意：在进一步操作前，应该将质押私钥储存在`/data/findora/{network_name}/{network_name}_node.key` 文件中。这将是你的`tmp.gen.keypair` 文件中的 `sec_key` 。如果该目录不存在，则需要自行创建。
...
## 安全清除[#](https://wiki.findora.org/docs/guides/auto-setup#safety-clean)

此脚本将不会清除你的验证人ID和钱包数据，只会清理数据并重新启动验证人。

### 设定环境路径变量[#](https://wiki.findora.org/docs/guides/auto-setup#set-environment-path-variables)
